...
export async function runSeed(tenantSlug: string, refs?: any) {
  try {
    ...
    const defaultGlobals = [
      { key: 'ALLOW_HTTP', value: 'true', description: 'Geliştirme ortamında HTTP çerezlerine izin verir', target: 'shared', type: 'boolean' },
      { key: 'TELEMETRY_VALIDATION_MODE', value: 'relaxed', description: 'Telemetri doğrulama modu (relaxed/strict)', target: 'api', type: 'string' },
      { key: 'SMTP_HOST', value: 'smtp.gmail.com', description: 'SMTP Sunucu Adresi', target: 'api', type: 'string' },
      { key: 'SMTP_PORT', value: '587', description: 'SMTP Bağlantı Noktası', target: 'api', type: 'number' },
      { key: 'SMTP_USER', value: '', description: 'SMTP Kullanıcı Adı', target: 'api', type: 'string' },
      { key: 'SMTP_PASS', value: '', description: 'SMTP Şifresi', target: 'api', type: 'password' },
      { key: 'SMTP_SECURE', value: 'false', description: 'SSL/TLS Kullanımı', target: 'api', type: 'boolean' },
      { key: 'SMTP_REQUIRE_TLS', value: 'true', description: 'TLS Zorunluluğu', target: 'api', type: 'boolean' },
      { key: 'SMTP_IGNORE_TLS', value: 'false', description: 'TLS İptali', target: 'api', type: 'boolean' },
      { key: 'EMAIL_FROM', value: 'noreply@example.com', description: 'E-Posta Gönderen Adresi', target: 'api', type: 'string' },
      { key: 'MQTT_ADMIN_USER', value: 'admin', description: 'MQTT Broker varsayılan admin kullanıcısı', target: 'api', type: 'string' },
      { key: 'MQTT_ADMIN_PASS', value: 'admin123', description: 'MQTT Broker varsayılan admin şifresi', target: 'api', type: 'password' },
      { key: 'MQTT_COMMAND_TIMEOUT', value: '30', description: 'MQTT Komut Zaman Aşımı Süresi (Saniye)', target: 'api', type: 'number' },
      { key: 'SANDBOX_TIMEOUT', value: '5', description: 'Sandbox Asenkron Çalışma Zaman Aşımı (Saniye)', target: 'api', type: 'number' },
      { key: 'APP_NAME', value: 'BaseApp', description: 'Uygulama Adı (PWA Manifest)', target: 'shared', type: 'string' },
      { key: 'LIGHT_PRIMARY', value: '#1976D2', description: 'Aydınlık Tema Ana Rengi', target: 'ui', type: 'color' },
      { key: 'DARK_PRIMARY', value: '#1976D2', description: 'Karanlık Tema Ana Rengi', target: 'ui', type: 'color' },
      { key: 'LIGHT_SECONDARY', value: '#424242', description: 'Aydınlık Tema İkincil Rengi', target: 'ui', type: 'color' },
      { key: 'DARK_SECONDARY', value: '#BDBDBD', description: 'Karanlık Tema İkincil Rengi', target: 'ui', type: 'color' },
      { key: 'LIGHT_BACKGROUND', value: '#FFFFFF', description: 'Aydınlık Tema Arkaplan Rengi', target: 'ui', type: 'color' },
      { key: 'DARK_BACKGROUND', value: '#121212', description: 'Karanlık Tema Arkaplan Rengi', target: 'ui', type: 'color' },
      { key: 'LIGHT_SURFACE', value: '#FFFFFF', description: 'Aydınlık Tema Kart Rengi', target: 'ui', type: 'color' },
      { key: 'DARK_SURFACE', value: '#1E1E1E', description: 'Karanlık Tema Kart Rengi', target: 'ui', type: 'color' },
      { key: 'APP_LOGO', value: '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="2730 7938 15540 13824"><defs><style>.fil0{fill:#03396C} .fil1{fill:#6497B1}</style></defs><g><polygon class="fil0" points="5013,10810 11456,8438 17147,10696 15008,11483 15602,11719 17741,10932 15602,11719 11456,10074 7152,11658 5013,10810 5771,10531 7880,11390 7152,11658 "/><polygon class="fil0" points="5042,17132 5042,18768 11327,21262 17770,18890 17770,17254 11327,19626 "/><polygon class="fil1" points="3230,13815 3230,15451 9516,17945 15958,15573 15958,13937 9516,16309 "/><polygon class="fil0" points="5013,10810 5013,12446 11299,14939 17741,12568 17741,10932 11299,13303 "/><polygon class="fil0" points="17214,17034 15075,17821 13229,17089 15368,16302 "/><polygon class="fil1" points="3859,13584 5998,14433 7534,13867 5395,13019 "/></g></svg>', description: 'Uygulama Logosu SVG Ham Kodu', target: 'shared', type: 'string' },
      
      // Master-Only Global Parametreler
      { key: 'WS_HEARTBEAT_INTERVAL', value: '30000', description: 'WebSocket Ping/Pong döngü süresi (ms)', target: 'api', type: 'number', is_master_only: true },
      { key: 'CRON_TICK_MS', value: '1000', description: 'Cron zaman tarama döngü hızı (ms)', target: 'api', type: 'number', is_master_only: true },
      { key: 'WORKER_CRASH_WINDOW_MS', value: '60000', description: 'Worker çökme sıklığı algılama aralığı (ms)', target: 'api', type: 'number', is_master_only: true },
      { key: 'MODBUS_IDLE_TIMEOUT_MS', value: '10000', description: 'Modbus TCP boşta kapanma süresi (ms)', target: 'api', type: 'number', is_master_only: true },
      { key: 'MODBUS_RESPONSE_TIMEOUT_MS', value: '3000', description: 'Modbus cevap bekleme süresi (ms)', target: 'api', type: 'number', is_master_only: true },
      { key: 'MODBUS_COOLDOWN_MS', value: '50', description: 'İki Modbus işlemi arası statik bekleme (ms)', target: 'api', type: 'number', is_master_only: true },


      // Tenant-Specific Parametreler
      { key: 'API_RATE_LIMIT', value: '1000', description: 'Kullanıcı başına dakikalık API istek limiti', target: 'api', type: 'number' },
      { key: 'LOGIN_RATE_LIMIT', value: '10', description: 'Dakikalık Login/Auth istek limiti (Brute-force)', target: 'api', type: 'number' },
      { key: 'CRON_WORKER_TIMEOUT_MS', value: '60000', description: 'Cron Fork yaşama süresi (ms)', target: 'api', type: 'number' },
      { key: 'SANDBOX_SCHEDULER_TIMEOUT', value: '900', description: 'Sandbox Scheduler çalışma limiti (sn)', target: 'api', type: 'number' },
      { key: 'WORKER_MEMORY_LIMIT_MB', value: '256', description: 'Fork edilen Worker RAM sınırı (MB)', target: 'api', type: 'number' },
      { key: 'TELEMETRY_BATCH_SIZE', value: '500', description: 'Telemetri DuckDB paket yazma limiti', target: 'api', type: 'number' },
      { key: 'API_RATE_LIMIT', value: '1000', description: 'Kullanıcı başına dakikalık API istek limiti', target: 'api', type: 'number' },
      { key: 'LOGIN_RATE_LIMIT', value: '10', description: 'Dakikalık Login/Auth istek limiti (Brute-force)', target: 'api', type: 'number' },
      { key: 'CRON_WORKER_TIMEOUT_MS', value: '60000', description: 'Cron Fork yaşama süresi (ms)', target: 'api', type: 'number' },
      { key: 'SANDBOX_SCHEDULER_TIMEOUT', value: '900', description: 'Sandbox Scheduler çalışma limiti (sn)', target: 'api', type: 'number' },
      { key: 'WORKER_MEMORY_LIMIT_MB', value: '256', description: 'Fork edilen Worker RAM sınırı (MB)', target: 'api', type: 'number' },
      { key: 'TELEMETRY_BATCH_SIZE', value: '500', description: 'Telemetri DuckDB paket yazma limiti', target: 'api', type: 'number' },
      { key: 'TELEMETRY_MAX_BUFFER_SIZE', value: '10000', description: 'Telemetri biriktirme OOM sınırı', target: 'api', type: 'number' },
      { key: 'DB_MAX_ACTIVE_OPERATIONS', value: '1000', description: 'SQLite eşzamanlı sorgu kilit limiti', target: 'api', type: 'number' },
      { key: 'DUCKDB_MEMORY_LIMIT', value: '256', description: 'DuckDB motoruna verilen RAM (MB)', target: 'api', type: 'number' }
    ];

    const globalsList = defaultGlobals.filter(g => !(g as any).is_master_only || tenantSlug === 'master');

    for (const globalVar of globalsList) {
      await mainSql.unsafe(`
        INSERT INTO globals (type, key, value, description, target, protected, data_type)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(key) DO NOTHING
      `, ['variable', globalVar.key, globalVar.value, globalVar.description, globalVar.target, 1, globalVar.type || 'string']);
    }
