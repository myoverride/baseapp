...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'PUT') {
    ...
    await sql.unsafe(`
      UPDATE workers SET 
        name = ?, type = ?, code = ?, cron_expression = ?, autostart = ?, active = ?, hashtags = ?, updated_by = ?, system_modified = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `, [
      body.name, body.type, body.code, body.cron_expression || null, body.autostart ? 1 : 0,
      body.active ? 1 : 0, hashtagsStr, user.id, isSystem, id
    ]);
