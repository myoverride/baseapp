...
export default defineEventHandler(async (event) => {
  ...
  let MAX_REQUESTS = isLogin ? 10 : 1000;
  if (pathname.startsWith('/api')) {
    const { globals } = await import('../utils/globalsManager');
    const querySlug = tenantSlug === 'global' ? 'master' : tenantSlug;
    if (isLogin) {
      MAX_REQUESTS = parseInt(await globals.get(querySlug, 'LOGIN_RATE_LIMIT', false, '10')) || 10;
    } else {
      MAX_REQUESTS = parseInt(await globals.get(querySlug, 'API_RATE_LIMIT', false, '1000')) || 1000;
    }
  }
