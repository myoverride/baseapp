<template>
  <g class="er-relation-line" :class="{ 'er-relation-line--hover': isHovered }">
    ...
    <path :d="pathD" fill="none" stroke="transparent" stroke-width="12" style="cursor: pointer;"
      @mouseenter="isHovered = true" @mouseleave="isHovered = false" @click="$emit('click', relation)" />
    <!-- Visible path -->
    <path :d="pathD" fill="none" :stroke="isHovered ? '#C62828' : '#90A4AE'" :stroke-width="isHovered ? 2.5 : 1.5"
      stroke-dasharray="none" :marker-end="isHovered ? `url(#${markerId}-active)` : `url(#${markerId})`"
      style="transition: stroke 0.2s, stroke-width 0.2s;" />
    <!-- Label -->
    <g v-if="label" :transform="labelTransform">
      <rect :x="-labelWidth / 2 - 4" :y="-7" :width="labelWidth + 8" height="14" rx="3"
        :fill="isHovered ? '#FFEBEE' : '#ECEFF1'" :stroke="isHovered ? '#C62828' : '#B0BEC5'" stroke-width="0.5" />
      <text text-anchor="middle" dominant-baseline="middle" :fill="isHovered ? '#C62828' : '#546E7A'" font-size="9"
