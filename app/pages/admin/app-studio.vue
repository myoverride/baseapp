              ...
              <v-col cols="12">
                <v-card variant="outlined" color="info" class="pa-4">
                  <div class="d-flex justify-space-between mb-2">
                    ...
                    <span class="font-weight-bold">{{ importProgress.percentage }}% ({{ importProgress.processed }} / {{ importProgress.total }})</span>
                  </div>
                  <v-progress-linear :model-value="importProgress.percentage" color="info" height="12" rounded striped :indeterminate="importProgress.total === 0"></v-progress-linear>
                  <v-alert v-if="importProgress.errors.length > 0" type="warning" density="compact" class="mt-4" icon="mdi-alert">
          ...
          <v-card class="fill-height d-flex flex-column">
            ...
            <v-card-text class="pa-4 flex-grow-1 d-flex flex-column" style="overflow-y: auto;">
              <p class="mb-4 text-body-1">
                <span v-html="$t('appStudio.importConfirmDesc1', { app: `<strong>${pendingPayload?.app_name || 'Uygulama'}</strong>` })"></span>
              </p>
            ...
            <v-card-actions class="pa-3">
              ...
              <v-btn :color="color" variant="elevated" @click="confirmImport" :loading="loading" :aria-label="$t('appStudio.confirmAndStart')">
