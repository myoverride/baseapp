<template>
  <v-container>
    ...
    <CrudTable ref="crudTable" api-endpoint="/api/admin/pages" :columns="columns" :title="$t('common.pages')"
      default-sort-key="created_at" default-sort-order="desc" @create="openCreateDialog" @edit="openEditDialog"
      @delete="handleDelete" :enable-multi-select="true" :extra-params="extraParams">
      ...
      <template v-slot:item.page_type="{ item }">
        <v-tooltip location="top">
          <template v-slot:activator="{ props }">
            <v-icon v-bind="props"
              :color="item.protected ? 'red' : (item.page_type === 'layout' ? 'purple' : item.page_type === 'component' ? 'pink' : 'primary')"
              size="small">
              <template v-if="item.protected">
                {{ item.route_pattern === '/' ? 'mdi-rocket-launch-outline' : (item.route_pattern === '/login' ?
                  'mdi-login' : 'mdi-lock-outline') }}
              </template>
              <template v-else>
                {{ item.page_type === 'layout' ? 'mdi-view-dashboard-outline' : item.page_type === 'component' ?
                  'mdi-puzzle-outline' : 'mdi-file-document-outline' }}
              </template>
            </v-icon>
          </template>
      ...
      <template v-slot:item.hashtags="{ item }">
        <div class="d-flex flex-wrap gap-1">
          <v-chip v-for="tag in safeJsonParse(item.hashtags)" :key="tag" size="x-small" :color="color" variant="tonal">
            {{ tag }}
          </v-chip>
        </div>
      </template>
    ...
    <ItemDialog ref="itemDialogRef" v-model="dialog" :mode="dialogMode" :initial-data="initialFormData" fullscreen
      @save="saveItem">
      <template #default="{ formData: fd }">
        <div class="d-flex flex-column fill-height"> <!-- Top Settings Area -->
          <div class="px-4 pt-3 flex-grow-0">
            <v-row density="compact">
              <!-- 1. Tip / Tür -->
              <v-col cols="12" md="2">
                <v-select v-model="fd.page_type" :items="pageTypes" item-title="title" item-value="value"
                  :label="$t('field.pageType')" variant="outlined" density="compact" hide-details
                  :readonly="fd.protected === 1 || fd.protected === true"></v-select>
              </v-col>

              <!-- 2. Sayfa Başlığı -->
              <v-col cols="12" md="3">
                <I18nTextField v-model="fd.title" @update:locale="previewLocale = $event" :label="$t('field.pageTitle')"
                  :placeholder="$t('field.pageTitle')" :required="true"></I18nTextField>
              </v-col>

              <!-- 3. Route Pattern -->
              <v-col cols="12" md="3" v-if="fd.page_type !== 'layout'">
                <v-text-field v-model="fd.route_pattern"
                  :label="fd.page_type === 'component' ? 'Component Slug' : $t('field.routePattern')"
                  :placeholder="fd.page_type === 'component' ? 'unique-component-slug' : '/path/:id'" variant="outlined"
                  density="compact" :readonly="fd.protected === 1 || fd.protected === true" :rules="[
              ...
              <!-- 4. Öncelik -->
              <v-col cols="12" md="1">
                <v-text-field v-model.number="fd.priority" :label="$t('field.priority')" type="number"
                  variant="outlined" density="compact" hide-details></v-text-field>
              </v-col>

              <!-- 5. Etiketler -->
              <v-col cols="12" md="3">
                <v-combobox v-model="fd.hashtags" :label="$t('field.hashtags')" multiple chips closable-chips
                  variant="outlined" density="compact" hide-details :items="[]" :delimiters="[',', ' ']"
                  append-inner-icon="mdi-tag-multiple"></v-combobox>
              </v-col>

              <!-- 6. Conditional: Layout Seçimi -->
              <v-col cols="12" md="4" v-if="!['layout', 'component'].includes(fd.page_type)">
                <v-select v-model="fd.layout_id" :items="allLayouts" item-title="displayTitle" item-value="id"
                  :label="$t('field.selectLayout')" variant="outlined" density="compact" clearable hide-details>
              ...
              <v-col cols="12" :md="['layout', 'component'].includes(fd.page_type) ? 12 : 8"
                class="d-flex align-center justify-end flex-wrap gap-4">
                <v-switch v-if="fd.page_type === 'layout'" v-model="fd.is_default_layout"
                  :label="$t('field.defaultLayout')" color="purple" density="compact" hide-details></v-switch>

                <v-switch v-if="fd.page_type !== 'component'" v-model="fd.is_public" :label="$t('common.public')"
                  color="blue" density="compact" hide-details></v-switch>

                <v-switch v-model="fd.active" :label="$t('common.active')" color="success" density="compact"
          ...
          <div class="flex-grow-1 d-flex flex-column px-4" style="min-height: 400px;">
            <div class="d-flex align-center justify-space-between px-2 py-1 bg-background border-bottom">
              <v-tabs v-model="tab" bg-color="transparent" :color="color" :grow="!mobile" density="compact">
                <v-tab value="template" :min-width="mobile ? 'auto' : undefined"><v-icon
                    :start="!mobile">mdi-vuejs</v-icon>
                  <span v-if="!mobile">{{ $t('tab.template') }}</span></v-tab>
                <v-tab value="script" :min-width="mobile ? 'auto' : undefined"><v-icon
                    :start="!mobile">mdi-language-javascript</v-icon> <span v-if="!mobile">{{ $t('common.code')
                    }}</span></v-tab>
                <v-tab value="style" :min-width="mobile ? 'auto' : undefined"><v-icon
                    :start="!mobile">mdi-language-css3</v-icon> <span v-if="!mobile">{{ $t('tab.style')
                    }}</span></v-tab>
                <v-tab value="preview" :min-width="mobile ? 'auto' : undefined"><v-icon
                    :start="!mobile">mdi-eye</v-icon> <span v-if="!mobile">{{ $t('tab.preview') }}</span></v-tab>
              </v-tabs>
            ...
            <div class="mt-2 flex-grow-1 position-relative">
              <div v-show="tab === 'template'" class="position-absolute w-100 h-100 border rounded"
                style="overflow: hidden;">
                <MonacoEditor v-model="fd.template_string" language="html" height="100%" contextType="frontend"
                  @save="saveCodeOnly" />
              </div>

              <div v-show="tab === 'script'" class="position-absolute w-100 h-100 border rounded"
                style="overflow: hidden;">
                <MonacoEditor v-model="fd.script_content" language="javascript" height="100%" contextType="frontend"
                  @save="saveCodeOnly" />
              </div>

              <div v-show="tab === 'style'" class="position-absolute w-100 h-100 border rounded"
                style="overflow: hidden;">
                <MonacoEditor v-model="fd.style_content" language="css" height="100%" contextType="frontend"
                  @save="saveCodeOnly" />
              </div>

              <div v-if="tab === 'preview'"
                class="position-absolute w-100 h-100 overflow-y-auto rounded border bg-background">
                <v-locale-provider :locale="previewLocale">
                  <DynamicRenderer :template-string="fd.template_string" :script-content="fd.script_content"
                    :style-content="fd.style_content" :route-params="{ id: 'PREVIEW_ID' }" :locale="previewLocale" />
                </v-locale-provider>
              </div>
            </div>
          </div>
    ...
    <CodeHistoryDialog v-model="historyDialogOpen" type="page" :id="editId || ''"
